...
class Advancedlocustscript01User(HttpUser):
                        ...
                        try:
                            ...
                            if user_ids_value is not None:
                                ...
                                if isinstance(user_ids_value, list):
                                    ...
                                    self.logger.info(f'Extracted array user_ids with {len(user_ids_value)} items')
                                else:
                                    self.variables['user_ids'] = str(user_ids_value)
                                    self.logger.info(f'Extracted user_ids = {self.variables["user_ids"]}')
                            else:
                                self.logger.warning(f'Failed to extract user_ids using JSONPath: $.users[*].id')
                            ...
                            if user_names_value is not None:
                                ...
                                if isinstance(user_names_value, list):
                                    ...
                                    self.logger.info(f'Extracted array user_names with {len(user_names_value)} items')
                                else:
                                    self.variables['user_names'] = str(user_names_value)
                                    self.logger.info(f'Extracted user_names = {self.variables["user_names"]}')
                            else:
                                self.logger.warning(f'Failed to extract user_names using JSONPath: $.users[*].name')
                            ...
                            if total_users_value is not None:
                                ...
                                if isinstance(total_users_value, list):
                                    ...
                                    self.logger.info(f'Extracted array total_users with {len(total_users_value)} items')
                                else:
                                    self.variables['total_users'] = str(total_users_value)
                                    self.logger.info(f'Extracted total_users = {self.variables["total_users"]}')
                            else:
                                self.logger.warning(f'Failed to extract total_users using JSONPath: $.total')
                        ...
                        try:
                            ...
                            if json_value is not None:
                                ...
                                if isinstance(json_value, list):
                                    if len(json_value) < 1:
                                        assertion_failures.append(f'Should have at least 1 user: list has {len(json_value)} items, which is below minimum 1')
                                else:
                                    if json_value < 1:
                                        assertion_failures.append(f'Should have at least 1 user: value {json_value} is below minimum 1')

                            else:
                                assertion_failures.append(f'Should have at least 1 user: JSONPath expression returned None')
                        except Exception as e:
                            assertion_failures.append(f'Should have at least 1 user: error evaluating JSONPath - {str(e)}')
                        ...
                        try:
                            ...
                            if user_products_value is not None:
                                ...
                                if isinstance(user_products_value, list):
                                    ...
                                    self.logger.info(f'Extracted array user_products with {len(user_products_value)} items')
                                else:
                                    self.variables['user_products'] = str(user_products_value)
                                    self.logger.info(f'Extracted user_products = {self.variables["user_products"]}')
                            else:
                                self.logger.warning(f'Failed to extract user_products using JSONPath: $.products[*].id')
                            ...
                            if user_orders_value is not None:
                                ...
                                if isinstance(user_orders_value, list):
                                    ...
                                    self.logger.info(f'Extracted array user_orders with {len(user_orders_value)} items')
                                else:
                                    self.variables['user_orders'] = str(user_orders_value)
                                    self.logger.info(f'Extracted user_orders = {self.variables["user_orders"]}')
                            else:
                                self.logger.warning(f'Failed to extract user_orders using JSONPath: $.orders[*].id')
                        ...
                        try:
                            ...
                            if product_categories_value is not None:
                                ...
                                if isinstance(product_categories_value, list):
                                    ...
                                    self.logger.info(f'Extracted array product_categories with {len(product_categories_value)} items')
                                else:
                                    self.variables['product_categories'] = str(product_categories_value)
                                    self.logger.info(f'Extracted product_categories = {self.variables["product_categories"]}')
                            else:
                                self.logger.warning(f'Failed to extract product_categories using JSONPath: $.categories[*]')
                            ...
                            if product_price_value is not None:
                                ...
                                if isinstance(product_price_value, list):
                                    ...
                                    self.logger.info(f'Extracted array product_price with {len(product_price_value)} items')
                                else:
                                    self.variables['product_price'] = str(product_price_value)
                                    self.logger.info(f'Extracted product_price = {self.variables["product_price"]}')
                            else:
                                self.logger.warning(f'Failed to extract product_price using JSONPath: $.price')
