...
class ComprehensiveEcommerceFlowUser(HttpUser):
    ...
    def load_test_data(self):
        ...
        self.test_data['user_credentials'] = [{'username': 'testuser1', 'password': 'password123', 'email': 'test1@example.com', 'role': 'customer'}, {'username': 'testuser2', 'password': 'password456', 'email': 'test2@example.com', 'role': 'customer'}, {'username': 'admin', 'password': 'admin123', 'email': 'admin@example.com', 'role': 'admin'}]

        # Load product_catalog data
        self.test_data['product_catalog'] = [{'id': 'PROD001', 'name': 'Laptop Computer', 'category': 'electronics', 'price': 999.99, 'description': 'High-performance laptop'}, {'id': 'PROD002', 'name': 'Wireless Headphones', 'category': 'electronics', 'price': 199.99, 'description': 'Noise-cancelling headphones'}, {'id': 'PROD003', 'name': 'Coffee Maker', 'category': 'home', 'price': 89.99, 'description': 'Programmable coffee maker'}]
    ...
    def run_scenario(self):
        ...
        try:
            ...
            # Extract session_id using regex: session_id=([a-zA-Z0-9]+)
            session_id_value = self._extract_regex(response.text, r'session_id=([a-zA-Z0-9]+)')
            if session_id_value:
                self.variables['session_id'] = session_id_value
                self.logger.info(f'Extracted {var_name} = {self.variables["session_id"]}')
        ...
        try:
            ...
            # Prepare request parameters
            params = {}
            params['query'] = self.replace_variables('{{name}}')
            params['category'] = self.replace_variables('{{category}}')
            body = None
        ...
        # Response time assertion
        if response.elapsed.total_seconds() * 1000 > 1500:
            assertion_failures.append(f'Search should be fast: response time {response.elapsed.total_seconds() * 1000:.0f}ms exceeds 1500ms')
        ...
        try:
            ...
            # Extract cart_total using JSONPath: $.total
            cart_total_value = self._extract_json_path(response_data, '$.total')
            if cart_total_value is not None:
                self.variables['cart_total'] = str(cart_total_value)
                self.logger.info(f'Extracted {var_name} = {self.variables["cart_total"]}')
        ...
        # Regex assertion
        if not re.search(r'CART-\d{6}', response.text):
            assertion_failures.append(f'Cart ID should match expected format: response does not match pattern "CART-\d{6}"')
        ...
        try:
            ...
            if json_value is not None:
                ...
                if json_value != 'pending':
                    assertion_failures.append(f'json_path assertion: expected 'pending', got {json_value}')
